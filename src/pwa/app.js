...
const BEGINNER_CLARITY_DATA_EPOCH_V356 = "20260813_v356_clarity1";
...
const conceptInfo = {
  "print": {
    definition: "print()는 괄호 안의 값을 화면에 보여 주는 함수다. print(name)이면 name에 저장된 값을 보여 주고, print(\"name\")이면 글자 name을 그대로 보여 준다.",
    example: "name = \"LiDAR\"\nprint(name)"
  },
  "len": {
    definition: "len(value)는 문자열의 글자 수나 리스트의 항목 수처럼 value 안에 몇 개가 들어 있는지 세어 정수로 돌려준다.",
    example: "items = [\"UAM\", \"ADAS\", \"Robotics\"]\nprint(len(items))"
  },
  "variable": {
    definition: "변수는 값을 나중에 다시 쓰기 위해 붙이는 이름이다. age = 20을 실행하면 이후 age라고 쓸 때 값 20을 사용할 수 있다.",
    example: "label = \"LiDAR\"\nname = label\nprint(name)"
  },
  "list": {
    definition: "리스트(list)는 여러 값을 순서대로 담는 자료다. 첫 번째 항목의 위치 번호는 0이고, 항목을 추가하거나 특정 위치의 값을 바꿀 수 있다.",
    example: "items = [\"UAM\", \"ADAS\", \"Robotics\"]\nprint(items[0])"
  },
  "dict": {
  ...
  "for": {
    definition: "for는 리스트나 문자열처럼 여러 항목이 있는 값에서 항목을 하나씩 꺼내 같은 블록을 반복 실행한다.",
    example: "items = [\"UAM\", \"ADAS\"]\nfor item in items:\n    print(item)"
  },
  "if": {
    definition: "if는 조건을 먼저 True 또는 False로 판단한다. 조건이 True이면 바로 아래에 들여쓴 코드를 실행하고, False이면 그 블록을 건너뛴다.",
    example: "kind = \"Sensor\"\nif kind == \"Sensor\":\n    print(\"sensor node\")"
  },
  "append": {
    definition: "list.append(value)는 value 하나를 기존 리스트의 맨 끝에 추가한다. 리스트 자체가 바뀌며 append()의 반환값은 None이다.",
...
Object.assign(conceptInfo, {
  ...
  "output": {definition: "출력은 프로그램이 계산한 결과가 화면이나 터미널에 보이는 것이다. 기초 문제에서는 print()가 무엇을 보여 주는지 확인하면 된다.", example: "print(\"hello\")"},
  "execution_order": {definition: "Python 코드는 보통 첫 줄부터 아래로 한 줄씩 실행된다. 각 줄이 실행될 때 변수에 어떤 값이 들어 있는지 순서대로 따라가면 최종 결과를 알 수 있다.", example: "x = 1\nx = 2\nprint(x)"},
  "assignment": {definition: "대입(assignment)은 = 오른쪽의 값을 먼저 계산한 뒤 그 결과를 왼쪽 변수에 저장하는 것이다. x = 3은 x에 3을 저장한다. x = x + 1은 현재 x 값에 1을 더한 결과를 다시 x에 저장한다.", example: "x = 3\nprint(x)"},
  "str": {definition: "문자열(str)은 글자를 다루는 값이다. 따옴표로 감싼 \"3\"은 숫자처럼 보여도 문자열이다. 문자열끼리 +를 사용하면 숫자 계산이 아니라 글자를 이어 붙인다.", example: "text = \"3\"\nprint(text)"},
  "int": {definition: "정수(int)는 3, -1, 100처럼 소수점이 없는 숫자다. int(\"10\")은 문자열 \"10\"을 숫자 10으로 바꾼다.", example: "number = int(\"3\")\nprint(number + 2)"},
  "float": {definition: "실수(float)는 3.5처럼 소수점이 있는 숫자를 다룰 때 쓰는 자료형이다. float(\"3.5\")은 문자열 \"3.5\"를 숫자 3.5로 바꾼다.", example: "value = float(\"3.5\")"},
  "type": {definition: "type(value)는 value가 어떤 자료형인지 확인한다. 예를 들어 type(3)은 int, type(\"3\")은 str인지 확인할 때 쓴다.", example: "print(type(\"3\"))"},
  "bool": {definition: "Boolean(bool)은 True와 False 두 값을 사용해 조건의 참과 거짓을 나타낸다. 비교식의 결과나 if 조건을 판단할 때 자주 나온다.", example: "active = True"},
  "comparison": {definition: "비교는 두 값을 확인해 True 또는 False를 만드는 연산이다. ==는 같은지, !=는 다른지, >=는 크거나 같은지 확인한다.", example: "print(3 < 5)"},
  "operator": {definition: "연산자는 값으로 계산이나 비교를 하는 기호다. +는 숫자에서는 덧셈, 문자열에서는 이어 붙이기로 동작할 수 있다.", example: "print(2 + 3)"},
  "else": {definition: "else는 바로 앞의 if나 elif 조건이 선택되지 않았을 때 실행할 코드를 적는 블록이다. 한 번의 if/else에서는 조건에 맞는 한쪽만 실행된다.", example: "if ready:\n    print(\"go\")\nelse:\n    print(\"wait\")"},
  "while": {definition: "while은 조건이 참인 동안 같은 코드 묶음을 반복한다. 반복 안에서 조건에 쓰는 값이 어떻게 바뀌는지 확인해야 한다.", example: "i = 0\nwhile i < 3:\n    i += 1"},
  "range": {definition: "range(stop)는 0부터 stop 바로 전까지의 정수를 만든다. range(start, stop)은 start부터 시작하고 stop은 포함하지 않는다.", example: "for i in range(3):\n    print(i)"},
  ...
  "index": {definition: "인덱스(index)는 리스트나 문자열에서 항목의 위치를 나타내는 번호다. Python에서는 첫 위치가 0이고 -1은 마지막 위치를 뜻한다.", example: "items = [\"a\", \"b\"]\nprint(items[0])"},
  ...
  "input": {definition: "input()은 사용자가 입력한 내용을 문자열로 받아 돌려주는 함수다. input(\"이름: \")의 괄호 안 문장은 안내 문구이고, 변수에는 사용자가 실제로 입력한 값이 저장된다.", example: "age = int(input())"},
  ...
  "print": {definition: "print()는 괄호 안의 값을 화면에 보여 주는 함수다. print(name)이면 name에 저장된 값을 보여 주고, print(\"name\")이면 글자 name을 그대로 보여 준다.", example: "name = \"Mina\"\nprint(name)"},
  "variable": {definition: "변수는 값을 나중에 다시 쓰기 위해 붙이는 이름이다. age = 20을 실행하면 이후 age라고 쓸 때 값 20을 사용할 수 있다.", example: "x = 3\nprint(x)"},
  "list": {definition: "리스트(list)는 여러 값을 순서대로 담는 자료다. 첫 번째 항목의 위치 번호는 0이고, 항목을 추가하거나 특정 위치의 값을 바꿀 수 있다.", example: "items = [\"a\", \"b\"]\nprint(items[0])"},
  "dict": {definition: "dict는 key라는 이름표와 value라는 값을 짝지어 저장하는 자료다. key를 사용해 원하는 값을 찾는다.", example: "data = {\"name\": \"Mina\"}\nprint(data[\"name\"])"},
  "for": {definition: "for는 리스트나 문자열처럼 여러 항목이 있는 값에서 항목을 하나씩 꺼내 같은 블록을 반복 실행한다.", example: "for item in [\"a\", \"b\"]:\n    print(item)"},
  "if": {definition: "if는 조건을 먼저 True 또는 False로 판단한다. 조건이 True이면 바로 아래에 들여쓴 코드를 실행하고, False이면 그 블록을 건너뛴다.", example: "if score >= 60:\n    print(\"pass\")"},
...
Object.assign(conceptInfo, {
  "print": {definition:"print()는 괄호 안의 값을 화면에 보여 주는 함수다. print(name)이면 name에 저장된 값을 보여 주고, print(\"name\")이면 글자 name을 그대로 보여 준다.", example:"name = \"Mina\"\nprint(name)"},
  "len": {definition:"len(value)는 문자열의 글자 수나 리스트의 항목 수처럼 value 안에 몇 개가 들어 있는지 세어 정수로 돌려준다.", example:"items = [\"a\", \"b\"]\nprint(len(items))"},
  "variable": {definition:"변수는 값을 나중에 다시 쓰기 위해 붙이는 이름이다. age = 20을 실행하면 이후 age라고 쓸 때 값 20을 사용할 수 있다.", example:"age = 20\nprint(age)"},
  "assignment": {definition:"대입(assignment)은 = 오른쪽의 값을 먼저 계산한 뒤 그 결과를 왼쪽 변수에 저장하는 것이다. x = 3은 x에 3을 저장한다. x = x + 1은 현재 x 값에 1을 더한 결과를 다시 x에 저장한다.", example:"x = 3\nprint(x)"},
  "type": {definition:"type(value)는 value가 어떤 자료형인지 확인한다. 예를 들어 type(3)은 int, type(\"3\")은 str인지 확인할 때 쓴다.", example:"print(type(3))\nprint(type(\"3\"))"},
  "str": {definition:"문자열(str)은 글자를 다루는 값이다. 따옴표로 감싼 \"3\"은 숫자처럼 보여도 문자열이다. 문자열끼리 +를 사용하면 숫자 계산이 아니라 글자를 이어 붙인다.", example:"text = \"3\"\nprint(text)"},
  "int": {definition:"정수(int)는 3, -1, 100처럼 소수점이 없는 숫자다. int(\"10\")은 문자열 \"10\"을 숫자 10으로 바꾼다.", example:"number = int(\"3\")\nprint(number + 2)"},
  "float": {definition:"실수(float)는 3.5처럼 소수점이 있는 숫자를 다룰 때 쓰는 자료형이다. float(\"3.5\")은 문자열 \"3.5\"를 숫자 3.5로 바꾼다.", example:"value = float(\"3.5\")\nprint(value)"},
  "bool": {definition:"Boolean(bool)은 True와 False 두 값을 사용해 조건의 참과 거짓을 나타낸다. 비교식의 결과나 if 조건을 판단할 때 자주 나온다.", example:"ready = True\nif ready:\n    print(\"go\")"},
  "list": {definition:"리스트(list)는 여러 값을 순서대로 담는 자료다. 첫 번째 항목의 위치 번호는 0이고, 항목을 추가하거나 특정 위치의 값을 바꿀 수 있다.", example:"items = [\"a\", \"b\"]\nprint(items[0])"},
  ...
  "for": {definition:"for는 리스트나 문자열처럼 여러 항목이 있는 값에서 항목을 하나씩 꺼내 같은 블록을 반복 실행한다.", example:"for item in [\"a\", \"b\"]:\n    print(item)"},
  "if": {definition:"if는 조건을 먼저 True 또는 False로 판단한다. 조건이 True이면 바로 아래에 들여쓴 코드를 실행하고, False이면 그 블록을 건너뛴다.", example:"score = 80\nif score >= 60:\n    print(\"pass\")"},
