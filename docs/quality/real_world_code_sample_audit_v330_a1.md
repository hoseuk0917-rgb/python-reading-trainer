...
- Total samples: 50
- Review-needed samples: 9
- Samples with generic step titles: 9
- Samples with unsupported items: 10
...
| html_basic_form | html | REVIEW | 4 | 0 | 4 | 4 | - |
| html_image_link | html | REVIEW | 3 | 0 | 3 | 3 | - |
| css_flex_card | css | REVIEW | 4 | 0 | 4 | 4 | - |
| css_media_query | css | REVIEW | 3 | 0 | 3 | 3 | - |
| sql_select_group | sql | REVIEW | 5 | 0 | 5 | 5 | - |
| sql_join_users_orders | sql | REVIEW | 4 | 0 | 4 | 4 | - |
...
- Step titles: 코드 실행 / 코드 실행 / 코드 실행 / 코드 실행
- Missing expected titles: -
- Unsupported conflicts: -
- First explanations:
  - 1. 이 줄을 순서대로 실행합니다.
...
- Step titles: 코드 실행 / 코드 실행 / 코드 실행 / 코드 실행 / 코드 실행
- Missing expected titles: -
- Unsupported conflicts: -
- First explanations:
  - 1. 이 줄을 순서대로 실행합니다.
...
- Step titles: 라이브러리 불러오기 / 객체 생성 결과 저장 / Python 코드 실행 / 함수 정의 / 값 돌려주기
- Missing expected titles: -
- Unsupported conflicts: -
- First explanations:
  - 1. 이미 만들어진 기능을 현재 코드에서 사용할 수 있게 가져옵니다.
  - 2. Flask 클래스로 새 객체를 만들고, 그 결과를 app 변수에 저장합니다. 이때 클래스의 __init__ 메서드가 객체의 초기값을 설정할 수 있습니다.
  - 3. 이 줄은 Python 코드입니다. 위에서 아래로 순서대로 실행됩니다.
  - 4. 나중에 이름으로 불러서 실행할 수 있는 코드 묶음을 만듭니다. 이 줄만으로 함수 안쪽이 바로 실행되지는 않습니다.
