...
- explanation: 'w'는 text 쓰기 모드다. 파일이 없으면 새로 만들고, 이미 있으면 open이 성공하는 순간 기존 길이를 0으로 줄여 내용을 비운다. 기존 자료를 보존해야 한다면 'a'나 안전한 임시 파일 교체 같은 다른 방식을 검토해야 한다. ‘w 모드 의미 고르기’에서는 open이 적용되는 줄과 그 전후의 변수 값을 위에서 아래로 연결해 보면 질문에서 요구한 최종 결과가 어떻게 만들어지는지 확인할 수 있다.
...
- explanation: 'a'는 append 모드로 쓰기를 파일 끝에 추가하고 파일이 없으면 만든다. 기존 끝에 줄바꿈이 없다면 새 문자열이 바로 이어질 수 있으므로 필요한 구분자나 개행은 코드가 직접 써야 한다. ‘a 모드 의미 고르기’에서는 open이 적용되는 줄과 그 전후의 변수 값을 위에서 아래로 연결해 보면 질문에서 요구한 최종 결과가 어떻게 만들어지는지 확인할 수 있다.
