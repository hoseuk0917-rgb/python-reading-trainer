...
Write-Host "=== LOCAL PRT PYTHON ARCHIFY EXECUTION ENDPOINT B2A ==="
...
if ($RemoteHead -ne $ExpectedBase) {
    throw "ABORT=UNEXPECTED_REMOTE_FEATURE_HEAD"
}
...
'@.TrimEnd()
    ...
    $RemainingEndpointPairCount = ([regex]::Matches($Text, [regex]::Escape($EndpointPairOld))).Count
    Write-Host "REMAINING_ENDPOINT_PAIR_ANCHOR_COUNT=$RemainingEndpointPairCount"
    if ($RemainingEndpointPairCount -ne 2) {
        throw "ABORT=REMAINING_ENDPOINT_PAIR_ANCHOR_COUNT_$RemainingEndpointPairCount"
    }
    ...
    Write-Host "RESULT=PASS_LOCAL_PRT_PYTHON_ARCHIFY_EXECUTION_ENDPOINT_B2A"
