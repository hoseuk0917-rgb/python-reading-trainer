...
Write-Host "=== V312 EXPLANATION ANSWER EXPLICIT PATCH VERIFY ==="
...
Invoke-NativeChecked "python compile audit" {
  python -m py_compile "tools\audit_explanation_answer_explicit_patch_v312.py"
}

Invoke-NativeChecked "run V312 audit" {
  python "tools\audit_explanation_answer_explicit_patch_v312.py"
}

Invoke-NativeChecked "lesson validation" {
  python "tools\validate_lessons.py" --expected-app-version 20260611_v312_a1 --expected-lesson-cards 1785
}

Write-Host "V312_EXPLANATION_ANSWER_EXPLICIT_PATCH_VERIFY_SCRIPT_OK"
