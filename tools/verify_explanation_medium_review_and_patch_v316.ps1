...
Write-Host "=== V316 EXPLANATION MEDIUM REVIEW AND PATCH VERIFY ==="
...
Invoke-NativeChecked "python compile audit" {
  python -m py_compile "tools\audit_explanation_medium_review_and_patch_v316.py"
}

Invoke-NativeChecked "run V316 audit" {
  python "tools\audit_explanation_medium_review_and_patch_v316.py"
}

Invoke-NativeChecked "lesson validation" {
  python "tools\validate_lessons.py" --expected-app-version 20260611_v316_a1 --expected-lesson-cards 1785
}

Write-Host "V316_EXPLANATION_MEDIUM_REVIEW_AND_PATCH_VERIFY_SCRIPT_OK"
