...
EXPECTED_VERSION = "20260611_v314_a1"
MARKER = "EXPLANATION_MEDIUM_REVIEW_AND_PATCH_V314_A1"
...
REVIEW = ROOT / "reports" / "explanation_medium_review_pack_v314.tsv"
CHANGE = ROOT / "reports" / "explanation_answer_explicit_patch_changes_v314.tsv"
SUMMARY_MD = ROOT / "reports" / "explanation_medium_review_and_patch_summary_v314.md"
AUDIT_MD = ROOT / "reports" / "explanation_answer_explicit_patch_audit_v314.md"
...
def main():
    ...
    assert_ok("ROOT_VERSION_V314", EXPECTED_VERSION in root_index)
    assert_ok("APP_VERSION_V314", f'const APP_DATA_VERSION = "{EXPECTED_VERSION}";' in app)
    assert_ok("STYLE_VERSION_V314", f"style.css?v={EXPECTED_VERSION}" in index)
    assert_ok("APP_SCRIPT_VERSION_V314", f"app.js?v={EXPECTED_VERSION}" in index)
    assert_ok("CODE_SCRIPT_VERSION_V314", f"code_explainer.js?v={EXPECTED_VERSION}" in index)
    assert_ok("COMMAND_SCRIPT_VERSION_V314", f"command_explainer.js?v={EXPECTED_VERSION}" in index)
    assert_ok("PROJECT_SCRIPT_VERSION_V314", f"project_analyzer.js?v={EXPECTED_VERSION}" in index)
    ...
    assert_ok("REVIEW_ROWS_120", len(review_rows) == 120, len(review_rows))
    assert_ok("CHANGE_ROWS_120", len(change_rows) == 120, len(change_rows))
    ...
    assert_ok("CHANGED_YES_120", sum(1 for r in change_rows if r["changed"] == "YES") == 120)
    assert_ok("V313_KEPT", "EXPLANATION_MEDIUM_REVIEW_AND_PATCH_V313_A1" in (ROOT / "reports" / "explanation_answer_explicit_patch_audit_v313.md").read_text(encoding="utf-8"))
    ...
    print("V314_EXPLANATION_MEDIUM_REVIEW_AND_PATCH_VERIFY_OK")
