...
Write-Host "=== V315 EXPLANATION MEDIUM REVIEW AND PATCH VERIFY ==="
...
Invoke-NativeChecked "python compile audit" {
  python -m py_compile "tools\audit_explanation_medium_review_and_patch_v315.py"
}

Invoke-NativeChecked "run V315 audit" {
  python "tools\audit_explanation_medium_review_and_patch_v315.py"
}

Invoke-NativeChecked "lesson validation" {
  python "tools\validate_lessons.py" --expected-app-version 20260611_v315_a1 --expected-lesson-cards 1785
}

Write-Host "V315_EXPLANATION_MEDIUM_REVIEW_AND_PATCH_VERIFY_SCRIPT_OK"
