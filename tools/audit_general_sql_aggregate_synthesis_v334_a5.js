...
const OUT_MD = path.join(ROOT, "docs", "quality", "general_sql_aggregate_synthesis_audit_v334_a5.md");
const OUT_JSON = path.join(ROOT, "docs", "quality", "general_sql_aggregate_synthesis_audit_v334_a5.json");
...
function runCase(analyze, sample) {
  const result = analyze(sample.code, "sql");
  ...
  checks.unshift(["summary_not_generic", !/코드를 \d+단계로 나눠 해석했습니다|쿼리를 \d+단계로 나눠 해석했습니다/.test(clean(result.summary))]);
  checks.push(["has_steps", steps.length >= sample.minSteps]);
  checks.push(["no_unknown_actions", actions.length === 0]);
  checks.push(["no_known_sql_unsupported", unsupported.length === 0]);
...
function main() {
  ...
  const samples = [
    {
      id: "sql_sales_sum_group_order_limit",
      title: "SQL SUM GROUP BY ORDER BY LIMIT",
      minSteps: 6,
      mustHave: ["sales", "status = 'paid'", "region", "SUM", "amount", "total_amount", "ORDER BY", "LIMIT 5", "상위 5개"],
      code: [
    ...
    {
      id: "sql_orders_count_group_order",
      title: "SQL COUNT GROUP BY ORDER BY",
      minSteps: 4,
      mustHave: ["orders", "category", "COUNT", "order_count", "ORDER BY", "큰 값부터 정렬"],
      code: [
  ...
  const lines = [];
  lines.push("# V334-A5 General SQL Aggregate Synthesis Audit");
  lines.push("");
  lines.push("Purpose: verify that non-preloaded SQL aggregate examples get synthesis explanations.");
  lines.push("");
  lines.push("## Summary");
  lines.push("");
  ...
  console.log("V334_A5_GENERAL_SQL_AGGREGATE_SYNTHESIS_AUDIT");
