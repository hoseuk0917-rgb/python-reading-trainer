...
Write-Host "=== V309 EXPLANATION MEDIUM REVIEW PACK VERIFY ==="
...
Invoke-NativeChecked "python compile audit" {
  python -m py_compile "tools\audit_explanation_medium_review_pack_v309.py"
}

Invoke-NativeChecked "run V309 audit" {
  python "tools\audit_explanation_medium_review_pack_v309.py"
}

Invoke-NativeChecked "lesson validation" {
  python "tools\validate_lessons.py" --expected-app-version 20260611_v309_a1 --expected-lesson-cards 1785
}

Write-Host "V309_EXPLANATION_MEDIUM_REVIEW_PACK_VERIFY_SCRIPT_OK"
