...
const baseUrl = process.env.V342_SMOKE_URL || "http://127.0.0.1:3377/tools/learning_hardening_v342_browser_case_r2.html";
...
if(!chrome){console.error("CHROME_FOUND=False");console.error("RESULT=FAIL_LEARNING_HARDENING_V342_REAL_BROWSER_SMOKE_R2");process.exit(1);}
console.log("=== PRT V342 HARDENING REAL BROWSER SMOKE RUNNER R2 ===");
console.log("CHROME_FOUND=True");
console.log("CHROME="+chrome);
console.log("HARNESS_BASE_URL="+baseUrl);

function runCase(caseName){
  const profile=fs.mkdtempSync(path.join(os.tmpdir(),"prt-v342-r2-"+caseName+"-"));
  const url=baseUrl+(baseUrl.includes("?")?"&":"?")+"case="+encodeURIComponent(caseName);
  const args=["--headless=new","--no-sandbox","--disable-dev-shm-usage","--disable-gpu","--disable-background-networking","--disable-component-update","--disable-default-apps","--disable-extensions","--disable-sync","--metrics-recording-only","--no-first-run","--window-size=1500,1800","--virtual-time-budget=240000","--user-data-dir="+profile,"--dump-dom",url];
  let result;
  try{result=spawnSync(chrome,args,{encoding:"utf8",maxBuffer:30*1024*1024,timeout:280000,env:{...process.env,HOME:profile}});}finally{try{fs.rmSync(profile,{recursive:true,force:true});}catch(_){}}
  ...
  const pass="RESULT=PASS_LEARNING_HARDENING_V342_REAL_BROWSER_CASE_R2";
  const fail="RESULT=FAIL_LEARNING_HARDENING_V342_REAL_BROWSER_CASE_R2";
  ...
  console.log("REPORT_PASS_MARKER_FOUND="+report.includes(pass));console.log("REPORT_FAIL_MARKER_FOUND="+report.includes(fail));
...
console.log("\nDESKTOP_V342_BROWSER_R2_PASS="+desktop);console.log("NARROW_V342_BROWSER_R2_PASS="+narrow);
if(!desktop||!narrow){console.error("RESULT=FAIL_LEARNING_HARDENING_V342_REAL_BROWSER_SMOKE_R2");process.exit(1);}
console.log("RESULT=PASS_LEARNING_HARDENING_V342_REAL_BROWSER_SMOKE_R2");
