...
Write-Host "=== V311 EXPLANATION MEDIUM REVIEW PACK VERIFY ==="
...
Invoke-NativeChecked "python compile audit" {
  python -m py_compile "tools\audit_explanation_medium_review_pack_v311.py"
}

Invoke-NativeChecked "run V311 audit" {
  python "tools\audit_explanation_medium_review_pack_v311.py"
}

Invoke-NativeChecked "lesson validation" {
  python "tools\validate_lessons.py" --expected-app-version 20260611_v311_a1 --expected-lesson-cards 1785
}

Write-Host "V311_EXPLANATION_MEDIUM_REVIEW_PACK_VERIFY_SCRIPT_OK"
